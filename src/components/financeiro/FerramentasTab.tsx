...
import { useToolsSubscriptions } from "../../hooks/useTorreControle";
...
function ToolModal({ onClose, onSave, editing }: { onClose: () => void; onSave: (t: Omit<ToolSubscription, "id" | "created_at" | "updated_at" | "deleted_at" | "created_by">) => void; editing: ToolSubscription | null }) {
...
export function FerramentasTab() {
  ...
  async function handleSave(t: Omit<ToolSubscription, "id" | "created_at" | "updated_at" | "deleted_at" | "created_by">) {
    if (showModal && showModal !== "new") {
      await updateItem(showModal.id, t);
    } else if (profile?.id) {
      await createItem({ ...t, created_by: profile.id });
    }
    setShowModal(null);
  }
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border)" }}>
                {["Nome", "Categoria", "Valor", "Ciclo", "Renovação", ""].map((h) => (
              ...
              {tools.map((t) => {
                ...
                return (
                  <tr key={t.id} style={{ borderBottom: "1px solid var(--bg-surface-2)", opacity: t.is_active ? 1 : 0.5 }}>
                    ...
                    <td className="px-4 py-3" style={{ color: "var(--text-secondary)" }}>{t.billing_cycle === "anual" ? "Anual" : "Mensal"}</td>
      ...
      {showModal && (
        <ToolModal onClose={() => setShowModal(null)} onSave={handleSave} editing={showModal === "new" ? null : showModal} />
      )}
    </div>
  );
}
